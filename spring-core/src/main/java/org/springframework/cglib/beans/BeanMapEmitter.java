...
import org.springframework.cglib.beans.BeanMapEmitter.1;
import org.springframework.cglib.beans.BeanMapEmitter.2;
import org.springframework.cglib.beans.BeanMapEmitter.3;
...
class BeanMapEmitter extends ClassEmitter {
	...
	public BeanMapEmitter(ClassVisitor v, String className, Class type, int require) {
		...
		if (require != 0) {
			Iterator allNames = allProps.keySet().iterator();
			...
			label26 : while (true) {
				...
				do {
					...
					name = (String) allNames.next();
				} while (((require & 1) == 0 || getters.containsKey(name))
						&& ((require & 2) == 0 || setters.containsKey(name)));
		...
		String[] allNames1 = this.getNames(allProps);
		this.generateKeySet(allNames1);
		this.generateGetPropertyType(allProps, allNames1);
		this.end_class();
	}
	...
	private void generateGet(Class type, Map getters) {
      CodeEmitter e = this.begin_method(1, BEAN_MAP_GET, (Type[])null);
      e.load_arg(0);
      e.checkcast(Type.getType(type));
      e.load_arg(1);
      e.checkcast(Constants.TYPE_STRING);
      EmitUtils.string_switch(e, this.getNames(getters), 1, new 1(this, getters, e));
      e.end_method();
   }

	private void generatePut(Class type, Map setters) {
      CodeEmitter e = this.begin_method(1, BEAN_MAP_PUT, (Type[])null);
      e.load_arg(0);
      e.checkcast(Type.getType(type));
      e.load_arg(1);
      e.checkcast(Constants.TYPE_STRING);
      EmitUtils.string_switch(e, this.getNames(setters), 1, new 2(this, setters, e));
      e.aconst_null();
      e.return_value();
      e.end_method();
   }
	...
	private void generateGetPropertyType(Map allProps, String[] allNames) {
      CodeEmitter e = this.begin_method(1, GET_PROPERTY_TYPE, (Type[])null);
      e.load_arg(0);
      EmitUtils.string_switch(e, allNames, 1, new 3(this, allProps, e));
      e.end_method();
   }

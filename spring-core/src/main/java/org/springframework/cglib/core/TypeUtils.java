...
public class TypeUtils {
	...
	public static String getClassName(Type type) {
		return isPrimitive(type)
				? (String) rtransforms.get(type.getDescriptor())
				: (isArray(type) ? getClassName(getComponentType(type)) + "[]" : type.getClassName());
	}
	...
	public static Type getUnboxedType(Type type) {
		return Constants.TYPE_INTEGER.equals(type)
				? Type.INT_TYPE
				: (Constants.TYPE_BOOLEAN.equals(type)
						? Type.BOOLEAN_TYPE
						: (Constants.TYPE_DOUBLE.equals(type)
								? Type.DOUBLE_TYPE
								: (Constants.TYPE_LONG.equals(type)
										? Type.LONG_TYPE
										: (Constants.TYPE_CHARACTER.equals(type)
												? Type.CHAR_TYPE
												: (Constants.TYPE_BYTE.equals(type)
														? Type.BYTE_TYPE
														: (Constants.TYPE_FLOAT.equals(type)
																? Type.FLOAT_TYPE
																: (Constants.TYPE_SHORT.equals(type)
																		? Type.SHORT_TYPE
																		: type)))))));
	}
	...
	public static int LCONST(long value) {
		return value == 0L ? 9 : (value == 1L ? 10 : -1);
	}

	public static int FCONST(float value) {
		return value == 0.0F ? 11 : (value == 1.0F ? 12 : (value == 2.0F ? 13 : -1));
	}

	public static int DCONST(double value) {
		return value == 0.0D ? 14 : (value == 1.0D ? 15 : -1);
	}
